...
df['time_diff_sec'] = df.groupby(['loc_id', 'date'])['time_sec'].diff().fillna(method='backfill')
...
# +
ser_co = df_daily.loc[df_daily['loc_id'] == 'CO11', ['date', 'avg_erythemal']].set_index('date').squeeze()
ser_nz = df_daily.loc[df_daily['loc_id'] == 'NZ01', ['date', 'avg_erythemal']].set_index('date').squeeze()
...
data_CO = df_daily.loc[df_daily['loc_id'] == 'CO11', ['date', 'avg_erythemal']].set_index('date').squeeze()
data_NZ = df_daily.loc[df_daily['loc_id'] == 'NZ01', ['date', 'avg_erythemal']].set_index('date').squeeze()
...
plot_anomaly(preds_CO, ax[0], ('2016-01-01', '2022-07-01'), title='Steamboat Springs, CO',
             ylabel='Avg Erythemal Irradiance, $W/m^2$')
plot_anomaly(preds_NZ, ax[1], ('2016-01-01', '2022-07-01'), title='Alexandra, NZ',
             ylabel='Avg Erythemal Irradiance, $W/m^2$')
